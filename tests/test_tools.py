...
class TestListBtpServices:
    ...
    @pytest.mark.asyncio
    @patch("server.get_service_offerings", new_callable=AsyncMock, return_value=FAKE_OFFERINGS)
    async def test_returns_all_services_no_keyword(self, _):
        from server import list_btp_services
...
class TestListBtpInstances:
    ...
    @pytest.mark.asyncio
    @patch("server.get_service_instances", new_callable=AsyncMock, return_value=FAKE_INSTANCES)
    async def test_returns_all_instances(self, _):
        from server import list_btp_instances
...
class TestGetBtpDestinations:
    ...
    @pytest.mark.asyncio
    @patch("server.get_destinations", new_callable=AsyncMock, return_value=FAKE_DESTINATIONS)
    async def test_returns_all_destinations(self, _):
        from server import get_btp_destinations
    ...
    @pytest.mark.asyncio
    @patch("server.get_destinations", new_callable=AsyncMock, return_value=[])
...
class TestRecommendBtpService:
    ...
    async def test_messaging_recommends_event_mesh(self, _):
        from server import recommend_btp_service
...
#
#     @pytest.mark.asyncio
#     @patch("server.get_entitlements", new_callable=AsyncMock, return_value=FAKE_ENTITLEMENTS)
#     async def test_returns_all_entitlements(self, _):
#         from server import get_btp_entitlements
